...
/// </summary>
public abstract unsafe class StandardDispatch : DisposableBase.Finalizable, IDispatch.Interface, IDispatchEx.Interface
{
    ...
    HRESULT IDispatchEx.Interface.GetDispID(BSTR bstrName, uint grfdex, int* pid)
    {
        ...
        *pid = PInvokeMadowaku.DISPID_UNKNOWN;
    ...
    /// <param name="id">
    ///  <see cref="PInvokeMadowaku.DISPID_STARTENUM"/> to start enumeration, or the last id returned by a previous call to
    ...
    HRESULT IDispatchEx.Interface.GetMemberProperties(int id, uint grfdexFetch, FDEX_PROP_FLAGS* pgrfdex)
    {
        ...
        if (id == PInvokeMadowaku.DISPID_UNKNOWN)
        {
