...
public static unsafe partial class ComHelpers
{
    ...
    internal static T* TryGetComPointer<T>(object? obj, out HRESULT result) where T : unmanaged, IComIID
    {
        ...
        result = ccw->QueryInterface(IID.Get<T>(), out void* ppvObject);
        ccw->Release();
        return (T*)ppvObject;
    }
    ...
    public static IUnknown* CreateComClass(Guid classId)
    {
        ...
        if (unknown is null)
        {
            Guid iid = IID.Get<IUnknown>();
            HRESULT hr = PInvokeMadowaku.CoCreateInstance(rclsid, null, CLSCTX.CLSCTX_INPROC_SERVER, &iid, (void**)&unknown);
            hr.ThrowOnFailure();
        }
        ...
        IUnknown* CreateWithIClassFactory2()
        {
            ...
            Guid iid = IID.Get<IClassFactory2>();
            HRESULT hr = PInvokeMadowaku.CoGetClassObject(rclsid, CLSCTX.CLSCTX_INPROC_SERVER, null, &iid, factory);
            ...
            if (info.fRuntimeKeyAvail)
            {
                ...
                factory.Pointer->CreateInstanceLic(null, IID.Get<IUnknown>(), key, out void* unknown);
                return (IUnknown*)unknown;
            }
            else
            {
                factory.Pointer->CreateInstance(null, IID.Get<IUnknown>(), out void* unknown);
                return (IUnknown*)unknown;
            }
        }
    }
        ...
        Guid interfaceId)
    {
        ...
        HRESULT hr = PInvokeMadowaku.LoadRegTypeLib(typeLibrary, majorVersion, minorVersion, 0, typelib);
        hr.ThrowOnFailure();
